...
export class PaymentController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Cập nhật thông tin tài khoản PayPal cho instructor' })
  async updateInstructorPaypalInfo(
    @Param('instructorId') instructorId: string,
    @Body() updateDto: UpdateInstructorPaypalDto,
    @Request() req,
  ) {
    this.logger.log(`Yêu cầu cập nhật PayPal: ${JSON.stringify({
    ...
    if (req.user.role !== 'ADMIN' && req.user.instructorId !== instructorId) {
      throw new UnauthorizedException('Bạn không có quyền cập nhật thông tin này');
    }

    return this.paymentService.updateInstructorPaypalInfo(instructorId, updateDto);
  }
  ...
  async getInstructorPaypalInfo(
    ...
    this.logger.log(`Yêu cầu xem PayPal: ${JSON.stringify({
      requestedInstructorId: instructorId,
      userRole: req.user.role,
      userInstructorId: req.user.instructorId
    })}`);
  ...
  async calculateInstructorPayout(
    ...
    this.logger.log(`Yêu cầu tính toán thanh toán: ${JSON.stringify({
      requestedInstructorId: instructorId,
      userRole: req.user.role,
      userInstructorId: req.user.instructorId
    })}`);
  ...
  async getMyInstructorInfo(@Request() req) {
    ...
    const instructorId = isInstructor ? await this.roleCheckService.getInstructorId(userId) : null;
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Kiểm tra trạng thái xác thực tài khoản PayPal của instructor hiện tại' })
  async getMyPaypalVerificationStatus(@Request() req) {
    ...
    const isTokenExpired = instructor.paypalVerificationTokenExp && 
    ...
    return {
      ...
      data: {
        ...
        hasPendingVerification: !!(instructor.paypalVerificationToken && !isTokenExpired),
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Gửi lại email xác nhận tài khoản PayPal cho instructor hiện tại' })
  ...
  @Roles('INSTRUCTOR')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Cập nhật giá tiền cho khóa học (yêu cầu xác minh PayPal)' })
  async updateCoursePrice(
    @Body() updateDto: UpdateCoursePriceDto,
    @Request() req,
  ) {
    const userId = req.user.userId || req.user.sub;
    this.logger.log(`Yêu cầu cập nhật giá khóa học: ${JSON.stringify({
